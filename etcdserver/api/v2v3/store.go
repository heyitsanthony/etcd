...
func NewStore(c *clientv3.Client) store.Store { return newStore(c) }

func newStore(c *clientv3.Client) *v2v3Store { return &v2v3Store{c, c.Ctx()} }
...
func (s *v2v3Store) Set(
	...
	applyf := func(stm concurrency.STM) error {
		...
		if !isRoot(parent) && stm.Rev(mkPath(parent)+"/") == 0 {
...
func (s *v2v3Store) Delete(nodePath string, dir, recursive bool) (*store.Event, error) {
	...
	return &store.Event{
		Action:    store.Delete,
		PrevNode:  mkV2Node(dresp.PrevKvs[0]),
		EtcdIndex: mkV2Rev(resp.Header.Revision),
	}, nil
}
...
func makeCompare(nodePath, prevValue string, prevIndex uint64) []clientv3.Cmp {
	key := mkPath(nodePath)
...
func mkPath(nodePath string) string { return mkPathDepth(nodePath, 0) }

func mkNodePath(p string) string { return path.Clean(p[len("/v2/k/000/"):]) }

func mkPathDepth(nodePath string, depth int) string {
...
func mkV2Node(kv *mvccpb.KeyValue) *store.NodeExtern {
	...
	n := &store.NodeExtern{
		Key:           string(mkNodePath(string(kv.Key))),
